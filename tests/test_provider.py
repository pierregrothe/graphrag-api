...
class TestProviderValidation:
    ...
    def test_provider_creation(self, provider: GraphRAGLLM, settings: Settings):
        ...
        expected_names = {LLMProvider.OLLAMA: "ollama", LLMProvider.GOOGLE_GEMINI: "google_gemini"}
    ...
    def test_implementation_patterns(self, provider: GraphRAGLLM, settings: Settings):
        ...
        if settings.llm_provider == LLMProvider.OLLAMA:
            ...
            assert required_keys.issubset(
                config.keys()
            ), f"Missing Ollama config keys: {required_keys - config.keys()}"

        elif settings.llm_provider == LLMProvider.GOOGLE_GEMINI:
            required_keys = {"api_key", "project_id", "llm_model", "embedding_model"}
            assert required_keys.issubset(
                config.keys()
            ), f"Missing Gemini config keys: {required_keys - config.keys()}"
    ...
    async def test_text_generation(self, provider: GraphRAGLLM):
        ...
        response = await provider.generate_text(
            prompt="What is 2+2? Answer with just the number.", max_tokens=10, temperature=0.1
        )
    ...
    async def test_embedding_consistency(self, provider: GraphRAGLLM):
        ...
        print(
            f"  Consistency Status: {'HIGH' if similarity > 0.99 else 'MODERATE' if similarity > 0.95 else 'LOW'}"
        )
    ...
    async def test_comprehensive_workflow(self, provider: GraphRAGLLM):
        ...
        response = await provider.generate_text(
            prompt="Explain AI in one sentence.", max_tokens=50, temperature=0.1
        )
