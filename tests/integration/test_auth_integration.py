    ...
    def test_api_key_creation_flow(self, client):
        ...
        if login_response.status_code == 200:
            ...
            if response.status_code == 201:
                ...
                assert "read:workspaces" in data["permissions"]
...
class TestWorkspaceAccessIntegration:
    ...
    def test_cross_workspace_access_prevention(self, client):
        ...
        invalid_workspaces = ["", " ", "workspace with spaces", "workspace/with/slashes"]
...
class TestErrorHandlingIntegration:
    ...
    def test_invalid_json_handling(self, client):
        ...
        for json_data in invalid_json_data:
            response = client.post(
                "/auth/login", content=json_data, headers={"Content-Type": "application/json"}
            )
    ...
    def test_large_request_handling(self, client):
        ...
        # Create large request data with unique email to avoid rate limiting interference
        large_data = {
            "email": "large_request_test@example.com",  # Unique email to avoid rate limiting
        ...
        # Include 429 as acceptable in case of rate limiting from other tests
