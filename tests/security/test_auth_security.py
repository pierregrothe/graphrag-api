...
from src.graphrag_api_service.auth.jwt_auth import JWTManager, JWTConfig, TokenData
...
from src.graphrag_api_service.auth.rate_limiting import RateLimiter, RateLimitConfig
...
class TestAPIKeySecurity:
    ...
    async def test_api_key_brute_force_protection(self, api_key_manager):
        ...
        for key in invalid_keys:
            result = await api_key_manager.validate_key(key)
    ...
    async def test_api_key_enumeration_protection(self, api_key_manager):
        ...
        for invalid_key in invalid_keys:
            result = await api_key_manager.validate_key(invalid_key)
            assert result is None
