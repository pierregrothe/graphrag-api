    ...
    def execute_request(self, request_item: dict) -> httpx.Response:
        ...
        if body_data.get("mode") == "raw":
            ...
            body = json.dumps({
                ...
                "variables": json.loads(graphql.get("variables", "{}"))
                if graphql.get("variables") else None
            })
    ...
    def run_collection(self, collection_path: str) -> dict:
        ...
        if failures:
            ...
            for failure in failures:
                print(f"  - {failure['folder']}/{failure['request']}: {failure.get('error', 'Test failures')}")
    ...
    def test_workspace_crud_operations(self, runner):
        ...
        update_request = {
            "request": {
                ...
                "body": {
                    "mode": "raw",
                    "raw": json.dumps({"description": "Updated description"})
                }
            }
        }
