...
class TestMasterKeyValidator:
    ...
    def master_key_validator(self, mock_settings):
        ...
        with patch('src.graphrag_api_service.auth.master_key.get_settings', return_value=mock_settings):
    ...
    def test_validate_admin_operation(self, master_key_validator):
        ...
        assert master_key_validator.validate_admin_operation("list_all_keys", key_manager_permissions)
        assert master_key_validator.validate_admin_operation("create_user_key", key_manager_permissions)
        assert not master_key_validator.validate_admin_operation("system_admin", key_manager_permissions)
...
class TestSecurityIntegration:
    ...
    def test_timing_attack_resistance(self):
        ...
        with patch('src.graphrag_api_service.auth.master_key.get_settings', return_value=mock_settings):
            ...
            start_time = time.time()
            result2 = validator.validate_master_key("grak_master_wrong_key_but_same_length_for_timing_test_here")
            ...
            if min(time1, time2) > 0:  # Avoid division by zero
                ...
                assert time_ratio < 10.0  # Should be within 10x of each other (relaxed for test environment)
            else:
                # If one time is zero, both should be very small
                assert max(time1, time2) < 0.001  # Less than 1ms
