...
class TestEnhancedJWTManager:
    ...
    def test_verify_token_with_invalid_token(self, jwt_manager):
        ...
        with pytest.raises(Exception):  # JWT library will raise various exceptions
    ...
    def test_verify_token_with_expired_token(self, jwt_manager):
        ...
        # This should still create the token (expiration is checked during verification)
        token = jwt_manager.create_access_token(expired_token_data)
    ...
    async def test_security_logging_integration(self, jwt_manager):
        ...
        with patch('src.graphrag_api_service.auth.jwt_auth.get_security_logger') as mock_logger:
            ...
            # Mock user data
            with patch.object(jwt_manager_with_mock, '_get_user_data', new_callable=AsyncMock) as mock_get_user:
                mock_get_user.return_value = {
                    "username": "testuser",
                    "email": "test@example.com",
                    "roles": ["user"],
