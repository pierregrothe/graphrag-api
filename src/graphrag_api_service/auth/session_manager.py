...
class SessionManager:
    ...
    async def create_session(
        ...
        self.security_logger.authentication_attempt(
            success=True, user_id=user_id, method="session_create"
        )
    ...
    async def get_session(self, session_id: str) -> Session | None:
        ...
        # Check idle timeout
        idle_time = datetime.now(UTC) - session.last_activity
    ...
    async def update_session_activity(
        self, session_id: str, ip_address: str | None = None, user_agent: str | None = None
    ) -> bool:
    ...
    async def revoke_session(self, session_id: str, reason: str = "manual") -> bool:
        ...
        self.security_logger.authentication_attempt(
            success=True, user_id=session.user_id, method="session_revoke"
        )

        return True

    async def revoke_user_sessions(self, user_id: str, except_session_id: str | None = None) -> int:
    ...
    async def cleanup_expired_sessions(self) -> int:
        ...
        for session_id, session in self._sessions.items():
            if (
                ...
                or (now - session.last_activity).total_seconds()
                > (self.config.max_idle_time_minutes * 60)
            ):
    ...
    async def get_session_by_refresh_token(self, refresh_token_id: str) -> Session | None:
    ...
    async def _flag_suspicious_activity(self, session: Session, activity_type: str) -> None:
        """Flag suspicious activity on session."""
        session.suspicious_activity_count += 1
        session.last_suspicious_activity = datetime.now(UTC)
