...
class RateLimiter:
    ...
    async def _check_fixed_window(
        ...
        if current_count >= config.requests_per_minute:
            ...
            raise RateLimitError(
                message=f"Rate limit exceeded: {current_count}/{config.requests_per_minute} requests per minute",
    ...
    async def _check_sliding_window(
        ...
        if len(window) >= config.requests_per_minute:
            ...
            raise RateLimitError(
                message=f"Rate limit exceeded: {len(window)}/{config.requests_per_minute} requests per minute",
                retry_after=retry_after,
                limit_type="requests_per_minute_sliding"
            )
    ...
    async def _cleanup_fixed_windows(self, identifier: str):
...
class EndpointRateLimiter:
    ...
    async def check_endpoint_rate_limit(
        ...
        config = custom_config or self.endpoint_configs.get(
            endpoint_type, 
            self.endpoint_configs["api"]
        )
        
        return await self.rate_limiter.check_rate_limit(
            f"{endpoint_type}:{identifier}",
            config,
            request
        )
