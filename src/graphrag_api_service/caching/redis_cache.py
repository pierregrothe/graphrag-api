...
class GraphRAGRedisCache:
    ...
    async def cache_entities(self, workspace_id: str, entities: List[Dict[str, Any]], ttl: int = 3600) -> bool:
        """Cache entity data.
    ...
    async def get_cached_entities(self, workspace_id: str) -> Optional[List[Dict[str, Any]]]:
...
_redis_cache: Optional[RedisDistributedCache] = None
_graphrag_cache: Optional[GraphRAGRedisCache] = None


async def get_redis_cache() -> Optional[RedisDistributedCache]:
...
async def get_graphrag_cache() -> Optional[GraphRAGRedisCache]:
