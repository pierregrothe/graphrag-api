...
class GraphQueryService(BaseService, GraphQueryServiceProtocol):
    ...
    def __init__(self, graph_operations, cache_service=None):
    ...
    async def get_entities(
        ...
        try:
            ...
            self.logger.info(f"Retrieved {len(result.get('entities', []))} entities for workspace {workspace_id}")
            return result
    ...
    async def get_relationships(
        ...
        try:
            ...
            # Execute query
            result = await asyncio.to_thread(
                self._execute_relationships_query,
                workspace_id,
                limit,
                offset,
                filters or {}
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_result = await asyncio.to_thread(
                self.graph_operations.test_connection
            )
    ...
    def _validate_workspace_id(self, workspace_id: str) -> None:
        ...
        if len(workspace_id) > 255:
            raise ValidationError(
                "Workspace ID too long (max 255 characters)",
                field="workspace_id"
            )
